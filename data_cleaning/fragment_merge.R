...
particle_count1 <- particle_count |>
  mutate(
...
particle_count1 <- particle_count1 |>
  mutate(ParticleID =
           case_when(
...
# Merge tables ----
merged_data <- left_join(particle_count1, full_particle, by = "ParticleID")
...
missing <- anti_join(particle_count1, fragment_data)
